...
prompt_user() {
    ...
    button=$("${NA_PATH}" \
        -type "${WINDOWTYPE}" \
        -popup_reminder "/timeinterval 5 /repeat" \
        -bar_title "${BAR_TITLE}" \
        -title "${TITLE}" \
        -subtitle "${SUBTITLE}" \
        -timeout "${TIMEOUT}" \
...
"
    ...
    if [ $RESPONSE -eq "0" ]; then
        ...
        curl -s https://raw.githubusercontent.com/grahampugh/erase-install/main/erase-install.sh | sudo bash /dev/stdin --reinstall --force-curl --rebootdelay 120 --current-user --depnotify --sameos
